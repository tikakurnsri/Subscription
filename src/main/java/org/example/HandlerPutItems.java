...
public class HandlerPutItems implements HttpHandler {
    private final HandlerPutCustomer.DatabaseConnection databaseConnection;

    public HandlerPutItems(HandlerPutCustomer.DatabaseConnection databaseConnection) {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        if ("PUT".equals(exchange.getRequestMethod())) {
            try {
                String response = handlePutRequest(exchange);
